...
I can finally say that the publication on my `Bioconductor` package `categoryCompare` [@CCPACKAGE] is finally published in the **Bioinformatics and Computational Biology** section of **Frontiers in Genetics** [@CCPUB]. This has been a long time coming, and I wanted to give some background on the inspiration and development of the method and software.
...
The original code used the `graphviz` package to do layouts for visualization, but it was difficult to install, and did not work the way I wanted. Thankfully Paul Shannon had just developed the `RCytoscape` package the year previous [@Shannon2013], and this enabled truly interactive visualization and passing data back and forth between `R` and `Cytoscape`. 
